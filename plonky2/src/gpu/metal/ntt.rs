...
// Import buffer tracking functions from poseidon_interleaved
use super::poseidon_interleaved::{track_allocation, track_deallocation};
...
#[cfg(feature = "metal-ntt")]
lazy_static! {
    static ref NTT_RUNTIME: MetalNTT = {
...
fn compute_primitive_root(n: usize) -> u64 {
    ...
    assert!(log_n <= GOLDILOCKS_TWO_ADICITY, "n exceeds field's two-adicity");
...
fn precompute_twiddles(device: &Device, max_log_n: usize) -> (Buffer, Buffer, Buffer) {
    ...
    // For size n, we need ω^0, ω^1, ..., ω^(n/2-1)
...
impl MetalNTT {
    ...
    pub fn warm_up(&self) {
        // Just accessing self triggers lazy_static initialization
    }
    ...
    fn ntt_in_place(&self, data_buffer: &Buffer, n: usize, log_n: usize, inverse: bool) {
        ...
        // So twiddles[i * twiddle_stride] gives omega_n^i where omega_n is the n-th root of unity
    ...
    pub fn lde(&self, values: &[GoldilocksField], rate_bits: usize) -> Vec<GoldilocksField> {
        ...
        assert!(extended_log_n <= self.max_log_n, "Extended size exceeds maximum supported");
    ...
    pub fn lde_onto_coset(&self, values: &[GoldilocksField], rate_bits: usize) -> Vec<GoldilocksField> {
        let n = values.len();
        assert!(n.is_power_of_two(), "LDE input size must be power of 2");
        let extended_n = n << rate_bits;
        let extended_log_n = extended_n.trailing_zeros() as usize;
