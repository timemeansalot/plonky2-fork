//! Recursive layout Merkle tree GPU implementation.
//!
//! This is the base implementation using the original plonky2 recursive digest layout.
...
use crate::gpu::metal::utils::{from_buf_raw, get_node_hash_index_in_digests, get_size_for_count};
...
impl MetalRuntime {
    pub fn new_merkle_tree(
        ...
        if std::env::var("PLONKY2_METAL_LOG").is_ok() {
            println!(
                "[GPU Merkle] Building tree: height={}, leaves={}, leaf_length={}, cap_height={}",
        ...
        if cap_height == tree_height || tree_height < 13 {
            if std::env::var("PLONKY2_METAL_LOG").is_ok() {
                println!("[GPU Merkle] Using CPU fallback");
            }
            return MerkleTree::new_from_2d(leaves, cap_height);
        }
        if std::env::var("PLONKY2_METAL_LOG").is_ok() {
            println!("[GPU Merkle] Using GPU");
        }
    ...
    pub fn hash_merkle_tree_buf_ho(
        ...
        // uniforms[5] = grid_width: total threads in X dimension for thread_id calculation
        let mut uniforms: [u32; 6] = [0, tree_length as u32, num_layers as u32, leaf_length as u32, leaf_count as u32, 0];

        // Use 256-byte aligned allocation for optimal memory coalescing
        // TrackedBuffer RAII ensures deallocation is tracked on drop
        let buffer_size = (total_digests * 4) * std::mem::size_of::<u64>();
        let digests_buffer = self.alloc_aligned_tracked(buffer_size);

        // CRITICAL: Zero the buffer to avoid reading uninitialized/stale data
        // Metal may reuse buffers from a pool with old data
        unsafe {
        ...
        let simd_width = pipeline_poseidon_hash_leaves.thread_execution_width() as usize;
        let max_threads = pipeline_poseidon_hash_leaves.max_total_threads_per_threadgroup() as usize;
        ...
        // Use TrackedBuffer for uniforms - RAII handles deallocation
        ...
        for i in 1..num_layers {
            ...
            // Use tuned threadgroup size, clamped to device limits
            let simd_width = pipeline_poseidon_hash_tree_level.thread_execution_width() as usize;
            let max_threads = pipeline_poseidon_hash_tree_level.max_total_threads_per_threadgroup() as usize;
            let num_threads = get_merkle_threadgroup_size(leaf_count, simd_width, max_threads) as u64;
        ...
        // TrackedBuffer RAII: digests_buffer and uniforms_buffer will auto-track
        // deallocation when they go out of scope here
        res
    }
}
